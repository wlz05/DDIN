...
class TripleModalFusionNetwork(nn.Module):
    def __init__(self, input_dim, output_dim):
        super(TripleModalFusionNetwork, self).__init__()
...
class MultiDomainPLEFENDModel(torch.nn.Module):
    def __init__(self, emb_dim, mlp_dims, bert, out_channels, dropout):
        ...
        checkpoint = torch.load('./model_weights/mae_pretrain_vit_{}.pth'.format(self.model_size), map_location='cpu')
        ...
        for i in range(self.domain_num):
            self.transformers_list.append(
                torch.nn.ModuleList([TransformerLayer(feature_emb_size, head_num=4, dropout=0.6,
                                                      attention_dropout=0,
                                                      initializer_range=0.02) for _ in
                                     range(self.layers)]))
            self.mlp_img_list.append(
                torch.nn.ModuleList([MLP_trans(img_emb_size, feature_emb_size, dropout=0.6) for _ in
                                     range(feature_num)]))
            self.mlp_text_list.append(
                torch.nn.ModuleList([MLP_trans(text_emb_size, feature_emb_size, dropout=0.6) for _ in
                                     range(feature_num)]))
